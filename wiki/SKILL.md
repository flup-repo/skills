...
Drop this file into .claude/skills/wiki/SKILL.md in any project. Put your data files in the project data folder. Run:

```
/wiki ingest        # Convert your data into raw markdown entries
/wiki absorb all    # Compile entries into wiki articles
/wiki query <q>     # Ask questions about the wiki (valuable answers become articles)
...
4. **Update and create articles.** Re-read every article before updating. Ask: **what new dimension does this entry add?** Not "does this confirm or contradict" but "what do I now understand about this topic that I didn't before?"
   ...
   **Standalone documents are not just sources.** When an entry is itself a substantive note, spec, debugging document, architecture writeup, test plan, meeting transcript, or ticket implementation memo, create or update a standalone article for that document unless it is obviously too small to matter. A broad project page may summarize and link it, but it must not be the only place the document survives.

   For technical work, the standalone article must preserve the implementation-level details that future queries will need:
   - what changed
   - why it changed
   - affected flows or acceptance criteria
   - files, services, config, or APIs involved
   - tests, verification steps, and open questions

   If a document has a ticket id, bug id, or other stable identifier, use it in the article title or aliases so later queries can find it directly.
...
- Never update existing wiki articles during a query. Only write new `query-result` articles.
- For specific implementation questions, search the generated wiki article bodies, not just `_index.md`. The index is for discovery; the answer should come from the most specific generated article available.

### Saving Query Results

Queries compound the wiki. A good answer is new knowledge — a synthesis that wouldn't appear in any single article. Save it.

**Save a query result when the answer:**
- Draws a comparison between two people, projects, eras, topics or themes
- Reveals a connection across 3+ articles that isn't captured anywhere
- Identifies a pattern emerging from synthesis that has no article yet
- Produces an analysis that would take significant re-reading to reconstruct

**Do NOT save when:**
- The answer is a simple factual lookup answered by one article
- The answer just summarizes a single existing article
- The wiki doesn't cover the topic (nothing to synthesize)

**How to save:**

After answering the question, write the result as a new wiki article in `wiki/queries/`. The article is the synthesis — not a transcript, not Q&A format. Write it as if it were any other wiki article.

Filename: slug the insight, e.g. `comparison-of-project-x-and-project-y.md`

```yaml
---
title: Descriptive Title (not the raw question)
type: query-result
query: "The original question as asked"
answered: YYYY-MM-DD
related: ["[[Source Article 1]]", "[[Source Article 2]]"]
sources: []
---
```

The title captures the insight, not the question. "How did X and Y differ?" becomes "X vs Y: Approach to Risk." The body is the synthesis, written to wiki standards — flat, factual, encyclopedic. No Q&A format, no "the user asked."

After writing the article, append the new entry to `_index.md`. Do not rebuild the full index.
...
| `queries/` | query-result | Cross-article syntheses generated by valuable queries |
...
| query-result | The synthesis (lead with insight), the evidence across articles, the connections or contrasts, any gaps |
